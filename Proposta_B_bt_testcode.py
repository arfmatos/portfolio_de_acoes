...
# Com esta função podemos extrair dados do data frame multiindex
...
# Traçar histórico de alocação de ativos
# Como entrada teremos um dataframe com as alocações
...
# Exclusão do dados com 0.0 do dataframe
...
# Calculo estatístico do portfólio com base na alocação
...
def calc_portfolio_values(alloc, stock_close):
    ...
    print('Retorno do Portfólio: {:.4f}'.format( np.sum(portfolio_returns )))
    print('Desvio Padrão: {:.4f}'.format(np.std(portfolio_returns)))
    print('Indice Sharpe: {:.4f}'.format( sharpe ))
...
# obtendo o nome do dicionário de parâmetros
def get_name(params):
    dc = ''
...
# Criando um dataframe do dicionário com estatísticas
...
# Plotando retorno de portfólio do dicionário com retornos
...
class DynRebalance(bt.SignalStrategy):
    ...
    def __init__(self):
        ...
        self.old_pct = []
        self.new_pct = []
        # Se o último elemento for benchmark, reduza o número de dados
        if(self.params.benchmarkUse == True):
            self.nDatas = len(self.datas)-1
        else:
            self.nDatas = len(self.datas)
        self.DataCounter = self.params.DataCounter
        self.RebalanceDay = self.params.RebalanceDay
        self.reserveCash = self.params.reserveCash

        self.verbose = self.params.printlog
...
def backtest(cfg, strategy, plot=False, m_name='Test', **kwargs):
    ...
    def getReturnAsDataFrame(pfa, bencha):
        ...
        t = pfa.get_analysis()
        lists = sorted(d.items()) # ordenado por chave, retorna uma lista de tuplas
        x, y = zip(*lists) # descompacte uma lista de pares em duas tuplas
        listst = sorted(t.items())
        x, y1 = zip(*listst)
    ...
    print('Saldo inicial do Portfólio: %.2f' % cerebro.broker.getvalue())
    thestrats = cerebro.run()
    thestrat = thestrats[0]
    print('Saldo final do  Portfólio: %.2f' % cerebro.broker.getvalue())
    if plot:
        cerebro.plot()
